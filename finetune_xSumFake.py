...
# Download dataset
with open('json_extraphraserank_1000.json') as json_file:
...
dataset_for_val = data_for_val.map(flatten, remove_columns=["id"])
dataset_for_val = dataset_for_val.map(list2samples, batched=True)

train_data_txt, _ = dataset.train_test_split(test_size=0.1).values()
_, validation_data_txt = dataset_for_val.train_test_split(test_size=0.1).values()
...
wandb_run = wandb.init(
        project="bart_for_sum_synthetic",
        config={
